...
impl BufferType {
    pub fn get(&self) -> u32 {
        match self {
            BufferType::ArrayBuffer => gll::ARRAY_BUFFER,
            BufferType::ElementArrayBuffer => gll::ELEMENT_ARRAY_BUFFER
        }
    }
}
...
pub fn buffer_data<T>(target: BufferTarget, vec: &Vec<T>, usage: BufferUsage) {
    unsafe {
        gll::BufferData(
            target.get(),
